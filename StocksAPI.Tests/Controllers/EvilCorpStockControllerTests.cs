...
public class EvilCorpStockControllerTests
{
    private readonly Mock<ILogger<EvilCorpStockController>> _mockLogger = new ();
    ...
    private readonly EvilCorpStock _evilCorpStock = new();

    [Fact]
    public async Task When_EvilCorpStockAPIIsCalled_Then_RetrieveDataAboutTheStock()
    {
        // Arrange
        ...
        _mockStocksDataRetriever.Setup(
            s => s.GetEvilCorpStockAsync()).ReturnsAsync(this._evilCorpStock);
        ...
        output.Value.Should().BeOfType<EvilCorpStock>();
        output.Value.Should().NotBeNull();
    }

    public EvilCorpStockController GetSut()
    {
        return new EvilCorpStockController(
